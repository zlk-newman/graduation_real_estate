...
public class MyUtils {
     ...
     * 单个文件上传
     *
     * @param file_aImg       文件流对象
     * @param userInfoBean       保存文件路径进行的实体类参数
     * @param serverFileSavePath 服务器保存文件的路径地址
     * @param fileNamePrefix     上传文件的前缀
     */
    public static void uploadSingleFile(MultipartFile file_aImg, Object userInfoBean, String serverFileSavePath, String fileNamePrefix) {
        ...
        // 获取文件类型
        String fileContentType = file_aImg.getContentType();
...
//        }
        ...
        // 将文件流写出到服务器保存路径
        try {
            file_aImg.transferTo(new File(serverSavePath + fileNewName));
        } catch (IOException e) {
        ...
        // 使用反射，将设置属性的方法具有通用性
        // 提取需要保存的属性名
        String propertyName = file_aImg.getName().replace("file_", "");
        try {
