    ...
    <script type="text/javascript">
        ...
        function doAjaxQuery() {
            ...
            $.ajax({
                ...
                url: "selectAllHistoryForP", // 和form表单的 action属性一样
                        ...
                        $('#myPaging').pagination({
                            ...
                            callback: function (eachPageData, pagination) { // 每次翻页的时候的回调函数
                                ...
                                $.each(eachPageData, function (index, eachValue) {
                                    ...
                                    strContent += "<td><button type=\"button\" class=\"btn btn-default\" onclick='deleteData(" + eachValue.hId + ")'>订单完成</button></td>";
                                    strContent += "</tr>";
                                });
                                // 显示每一页的内容
                                // 将拼接内容显示到指定的控件中
        ...
        function deleteData(hId) {
            // 使用ajax删除数据
            var r = confirm("是否确定完成该订单？")
            if (r == true) {
                $.post("deleteHistoryByID2", {"hId": hId}, function (data) {
                    // 可以选择弹出框进行提示，也可以不作处理
                    if (data && data == "1") {
                        // 删除成功
                        alert("删除成功");
