				...
				<div class="row">
					<div class="col-sm-6 auth-box">
						<div class="lochana-box-shadow">
							<!-- Page Title -->
							<h3 class="widget-title">添加商品房</h3>
							<!-- /Page Title -->

							<!-- Form -->
							<!-- action表示将表单提交到哪里-->
							<form id="form" action="${pageContext.request.contextPath}/saveCom" method="post" class="widget-form" enctype="multipart/form-data">

                                <div class="form-group row">
                                    <div class="col-sm-12">
                                        <img width="64" height="64" id="img" src="" />
                                        <input type="file" class="form-control" onchange="changepic(this)" id="comimg" accept="image/png,image/jpeg" name="file_comImg" placeholder="请选择商品房图片">
                                    </div>
                                </div>
                                ...
                                <div class="form-group row">
                                    <div class="col-sm-12">
                                        <input name="comPrice" id="comPrice" placeholder="价格" onkeyup="if( ! /^\d*(?:\.\d{0,2})?$/.test(this.value)){alert('只能输入数字，小数点后只能保留两位');this.value='';}"  class="form-control" required>
                                    </div>
                                </div>
								<div class="form-group row">
									<div class="col-sm-12">
                                ...
                                <div class="form-group row">
                                    <div class="col-sm-12">
                                        <input name="comNumber" placeholder="房产数量" onkeyup="if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,'')}else{this.value=this.value.replace(/\D/g,'')}" onafterpaste="if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,'')}else{this.value=this.value.replace(/\D/g,'')}" id="comNumber" type="text" class="form-control" required>
                                    </div>
                                </div>

                                <div class="form-group row">
                                    <div class="col-sm-12">
                                        <textarea name="comDescribe" placeholder="房产描述" id="comDescribe" rows=”3” class="form-control" cols=”10” style="resize: none"></textarea>
                                </div>
                                </div>
                            ...
                            <div class="button-btn-block">
                                <a href="${pageContext.request.contextPath}/forward?pg=productInfoManager/ComAjaxQueryWithPagination" ><button   class="btn btn-primary btn-lg btn-block">返回</button></a>
                            </div>
							<!-- /Form -->
						</div>
					</div>
