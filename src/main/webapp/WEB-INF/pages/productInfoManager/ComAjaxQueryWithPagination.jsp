    ...
    <script type="text/javascript">
        ...
        function doAjaxQuery() {
            ...
            $.ajax({
                ...
                url: "selectComByParam", // 和form表单的 action属性一样
                        ...
                        $('#myPaging').pagination({
                            ...
                            callback: function (eachPageData, pagination) { // 每次翻页的时候的回调函数
                                ...
                                $.each(eachPageData, function (index, eachValue) {
                                    ...
                                    strContent += "<td><button type=\"button\" class=\"btn btn-default\" onclick='deleteData(" + eachValue.comId + ")'>删除</button></td>";
                                    strContent += "</tr>";
                                });
                                // 显示每一页的内容
                                // 将拼接内容显示到指定的控件中
        ...
        function showModal4Modify(eachValue) {
            ...
            var targetUrl = "forward?pg=productInfoManager/ComRoomModify";
        ...
        function InitModal3(targetUrl) {
            ...
            Modal3.OnfirstInited = function (thismodal) {
                ...
                $("#btn1").click(function () {
                    ...
                    var hname = $("#modal3 iframe").contents().find("#comName").val();
                    var hprice=$("#modal3 iframe").contents().find("#comPrice").val();
                    var hnum=$("#modal3 iframe").contents().find("#comNumber").val();
                    var hadd=$("#modal3 iframe").contents().find("#comAddress").val();
                    var hdes=$("#modal3 iframe").contents().find("#comDescribe").val();
        ...
        ////////////////////////////////////////////////////////////////////////////////////////////////////
        function deleteData(comId) {
            // 使用ajax删除数据
            var r = confirm("是否确定删除该记录？")
            if (r == true) {
                $.post("deleteCommodityHouseByID", {"comId": comId}, function (data) {
                    // 可以选择弹出框进行提示，也可以不作处理
                    if (data && data == "1") {
                        // 删除成功
                        alert("删除成功");
...
<div id="maindiv" style="margin: 10px auto">
    <div id="formdiv">
        <div id="p_div1">
            <button><a href="${pageContext.request.contextPath}/forward?pg=productInfoManager/InsertComRoom">添加商品房信息</a></button>
        </div>
